...
<div class="p-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">Sent today</p>
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($sentToday) }}</p>
            @if($failedToday > 0)
                <p class="text-[13px] text-red-500 mt-1">{{ $failedToday }} failed</p>
            @else
                <p class="text-[13px] text-gray-300 mt-1">&nbsp;</p>
            @endif
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">This week</p>
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($sentWeek) }}</p>
            <p class="text-[13px] text-gray-300 mt-1">&nbsp;</p>
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">This month</p>
            <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($sentMonth) }}</p>
            <p class="text-[13px] text-gray-300 mt-1">&nbsp;</p>
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">Active users (30d)</p>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">Logins today</p>
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ number_format($loginsToday) }}</p>
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">Failed logins today</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">Groups</p>
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ number_format($groupCount) }}</p>
        </div>

        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-[13px] font-medium text-gray-400 uppercase tracking-wide">Mode</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('admin.logs.index') }}" class="text-[13px] text-orange-500 hover:text-orange-700">View all</a>
            </div>
                        ...
                        @foreach($recentSends as $row)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2.5">
                                    <p class="text-[13px] font-medium text-gray-800 truncate max-w-[180px]">{{ $row->email }}</p>
                                    <p class="text-[11px] text-gray-400 truncate max-w-[180px]">{{ $row->subject ?: '(no subject)' }}</p>
                                </td>
                                <td class="px-3 py-2.5 text-right whitespace-nowrap">
                                    @if($row->status === 'failed')
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-medium bg-red-50 text-red-600">failed</span>
                                    @endif
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('admin.logs.index', ['tab' => 'login']) }}" class="text-[13px] text-orange-500 hover:text-orange-700">View all</a>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach($recentLogins as $row)
                            <tr class="hover:bg-gray-50 {{ $row->success ? '' : 'bg-red-50/50' }}">
                                <td class="px-4 py-2.5">
                                    <p class="text-[13px] font-medium {{ $row->success ? 'text-gray-800' : 'text-red-700' }} truncate max-w-[180px]">
                                        {{ $row->email }}
                                    </p>
                                ...
                                <td class="px-3 py-2.5 text-right whitespace-nowrap">
                                    @if(!$row->success)
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-medium bg-red-100 text-red-600">failed</span>
                                    @else
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-medium bg-green-50 text-green-600">ok</span>
                                    @endif
