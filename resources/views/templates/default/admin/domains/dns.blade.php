...
<div class="p-6 max-w-3xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Mail exchange servers for this domain.</p>
                </div>
                @if($mx['found'])
                ...
                @if($mx['found'])
                    <ul class="space-y-1">
                        @foreach($mx['records'] as $record)
                            <li class="font-mono text-xs text-gray-700">{{ $record }}</li>
                        @endforeach
                    </ul>
                @else
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Specifies which servers are allowed to send email.</p>
                </div>
                @if($spf['found'])
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700">
                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414L8.414 15l-4.707-4.707a1 1 0 011.414-1.414L8.414 12.172l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
            ...
            <div class="px-5 py-4">
                @if($spf['found'])
                    <code class="block bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs font-mono text-gray-800 break-all">{{ $spf['record'] }}</code>
                @else
                    <p class="text-sm text-gray-400">No SPF TXT record found. Add one to prevent spoofing.</p>
                    <p class="text-xs text-gray-400 mt-2">Example: <code class="bg-gray-100 px-1 rounded">v=spf1 mx ~all</code></p>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Policy for handling authentication failures.</p>
                </div>
                @if($dmarc['found'])
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700">
                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414L8.414 15l-4.707-4.707a1 1 0 011.414-1.414L8.414 12.172l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
            ...
            <div class="px-5 py-4">
                @if($dmarc['found'])
                    <code class="block bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs font-mono text-gray-800 break-all">{{ $dmarc['record'] }}</code>
                @else
                    ...
                    <p class="text-xs text-gray-400 mt-2">Example: <code class="bg-gray-100 px-1 rounded">v=DMARC1; p=none; rua=mailto:dmarc@{{ $mail_domain->domain }}</code></p>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('admin.dkim.index') }}"
                   class="text-xs text-orange-500 hover:text-orange-700 transition-colors">Manage keys →</a>
            </div>
            <div class="px-5 py-4">
                @if(empty($dkim))
                    ...
                    <ul class="space-y-3">
                        @foreach($dkim as $entry)
                            <li class="flex items-center justify-between gap-4">
                                <p class="text-xs font-mono text-gray-700">{{ $entry['host'] }}</p>
                                @if($entry['published'])
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700 flex-shrink-0">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414L8.414 15l-4.707-4.707a1 1 0 011.414-1.414L8.414 12.172l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                        Published
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700 flex-shrink-0">
                                        Not published
                                    </span>
                                @endif
                            </li>
    ...
    <p class="mt-4 text-xs text-gray-400">DNS lookup performed server-side at {{ now()->format('H:i:s') }} UTC. Results may be cached by your server's resolver.</p>
